...
HOME_DIR=/app
GETH_CONFIG_DIR=$HOME_DIR/ethereum
GETH_DATA_DIR=/data
: ${GETH:=}
: ${FROM_SCRATCH:=false}
: ${SNAPSHOT_URL:=}

# If a remote geth RPC node URL is specified, the snapshot synchronization will be skipped
if [ -n "${GETH}" ]; then
    FROM_SCRATCH=false
fi
...
# If GETH is not specified, the geth node will be started by rosetta-core.
# We need to prepare everything to start the geth node.
if [[ ! -n "$GETH" ]]; then
...
echo "start rosetta-core"
exec /app/rosetta-core run